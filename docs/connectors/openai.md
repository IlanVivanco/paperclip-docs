...
seo_description: Supplies the credential Paperclip uses to run OpenAI models. It is not an agent tool, so it adds no actions to any permission list.
---
...
OpenAI is a **model provider**, not an agent tool. It supplies the credential an agent's runtime uses when it runs a model. It does not add any actions to the **Permissions** tab, and it never appears in a review request.
