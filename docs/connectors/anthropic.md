...
seo_description: Supplies the credential Paperclip uses to run Claude models. A subscription signs in with your plan; an API key can be scoped and rotated.
---

# Anthropic

Run Claude models. A subscription signs in with your Claude plan; an API key can be scoped and rotated separately.

## What this connector does

Anthropic is a **model provider**, not an agent tool. It supplies the credential an agent's runtime uses when it runs a model. It does not add any actions to the **Permissions** tab, and it never appears in a review request.
...
Open **Connectors**, find **Anthropic**, and select **Connect**. Paperclip offers exactly the paths below; no other setup path is supported.

### Claude subscription (`ai-subscription`)
